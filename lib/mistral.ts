...
export async function generateConversationalFollowUp(
  ...
  try {
    const prompt = `
    You are MediKiosk's empathetic, clinical conversational intake agent for an Outpatient Clinic in India.
    Your job is to conduct a natural, intelligent conversational medical history interview with the patient.
    
    PATIENT CONVERSATION HISTORY SO FAR:
    ${JSON.stringify(history, null, 2)}
    
    CURRENT TURN COUNT: ${turnCount} / 6
    PATIENT CHOSEN LANGUAGE: ${langConfig.name} (${langConfig.native})

    CLINICAL OBJECTIVES (SOCRATES Framework):
    1. Chief Complaint & Onset (When did it start? How did it develop?)
    2. Character & Severity (What does it feel like? Pain scale 1-10 or mild/moderate/severe?)
    3. Radiation & Associated Symptoms (Does it spread anywhere? Any nausea, sweating, dizziness, breathing issues?)
    4. Triggers & Relieving Factors (What makes it worse or better?)
    5. Medications & Known Allergies (What medicines do they take? Any drug allergies?)

    INSTRUCTIONS:
    - Based on what the patient just answered, dynamically formulate the next logical, intelligent clinical follow-up question.
    - Tailor the question specifically to the symptom they stated.
    - Output "question_localized" in ${langConfig.name} (${langConfig.native}) using proper script and natural phrasing.
    ...
    - If ${turnCount} >= 5, or if you have collected sufficient intake details, you can set "is_intake_complete": true.
    ...
    Output strictly valid JSON with no markdown:
    {
      ...
      "question_localized": "Next question in ${langConfig.name} script",
      ...
      "options": [
        "Option in ${langConfig.name} / English 1",
        "Option in ${langConfig.name} / English 2",
        "Option in ${langConfig.name} / English 3",
        "Option in ${langConfig.name} / English 4"
      ],
    ...
    return {
      ...
      question_localized: `${langConfig.native}: क्या आपको कोई अन्य लक्षण या दर्द महसूस हो रहा है?`,
      question_en: 'Do you feel any other symptoms or pain?',
      section: 'hpi',
      field_name: 'associated_symptoms',
      options: ['हाँ / Yes', 'नहीं / No', 'पता नहीं / Not sure'],
 ...
 */
export async function generateBilingualSummary(structuredHistory: any[], extractedEntities: any[], patientLangCode: string = 'hi') {
  const langConfig = SUPPORTED_LANGUAGES[patientLangCode] || SUPPORTED_LANGUAGES.hi;
  ...
  try {
    ...
    Output strictly JSON matching this structure:
    {
      ...
      "clinician_summary": {
        ...
        "medications": "Current medications list string",
    ...
    const meds = extractedEntities.filter(e => e.entity_type === 'medication').map(e => e.fields?.name || e.name).join(', ');
    ...
    return {
      ...
      clinician_summary: {
        ...
        medications: meds || 'None recorded',
